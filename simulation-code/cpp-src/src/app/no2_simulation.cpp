...
void write_global_average_to_file(double global_avg, const std::string& filepath) {
    ...
    results_file << "Global Average no2 Concentration: " << global_avg << std::endl;
}

// Function to check for high no2 concentration and alert
void alert_high_no2_concentration(double global_avg, double threshold = 30.0) {
    if (global_avg > threshold) {
        std::cout << "Alert: High no2 concentration detected: " << global_avg << std::endl;
    }
}
// Function to read no2 data from a CSV file
std::vector<double> read_no2_data(const std::string& filepath, int targetColumn) {
    std::vector<double> no2Values;
...
int main(int argc, char** argv) {
    ...
    if (shm_rank == 0) {
        ...
        if (world_rank == 0) {
            int targetColumn = 19;
            allData = read_no2_data("preprocessed_for_cpp.csv", targetColumn);
    ...
    if (world_rank == 0) {
        ...
        std::cout << "Global Average NO2 Concentration: " << global_avg << std::endl;
        
        write_global_average_to_file(global_avg, "no2_analysis_results.txt");

        alert_high_no2_concentration(global_avg);
    }
