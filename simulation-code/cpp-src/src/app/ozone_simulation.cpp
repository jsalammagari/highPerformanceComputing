...
// Function to read Ozone data from a CSV file
std::vector<double> read_ozone_data(const std::string& filepath, int targetColumn) {
    std::vector<double> ozoneValues;
...
int main(int argc, char** argv) {
    ...
    if (shm_rank == 0) {
        ...
        if (world_rank == 0) {
            int targetColumn = 18;
            allData = read_ozone_data("preprocessed_for_cpp.csv", targetColumn);
    ...
    if (world_rank == 0) {
        ...
        std::cout << "Global Average Ozone Concentration: " << global_avg << std::endl;
    }
