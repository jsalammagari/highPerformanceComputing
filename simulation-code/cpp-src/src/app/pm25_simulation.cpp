...
void write_global_average_to_file(double global_avg, const std::string& filepath) {
    ...
    results_file << "Global Average PM2.5 Concentration: " << global_avg << std::endl;
}

// Function to check for high PM10 concentration and alert
void alert_high_pm25_concentration(double global_avg, double threshold = 10.0) {
    if (global_avg > threshold) {
        std::cout << "Alert: High PM2.5 concentration detected: " << global_avg << std::endl;
    }
}

// Function to read PM2.5 data from a CSV file
std::vector<double> read_pm25_data(const std::string& filepath, int targetColumn) {
    std::vector<double> pm25Values;
...
int main(int argc, char** argv) {
    ...
    if (shm_rank == 0) {
        ...
        if (world_rank == 0) {
            int targetColumn = 17;
            allData = read_pm25_data("preprocessed_for_cpp.csv", targetColumn);
    ...
    if (world_rank == 0) {
        ...
        std::cout << "Global Average PM2.5 Concentration: " << global_avg << std::endl;
        
        write_global_average_to_file(global_avg, "pm25_analysis_results.txt");

        alert_high_pm25_concentration(global_avg);
    }
