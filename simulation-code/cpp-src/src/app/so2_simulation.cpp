...
void write_global_average_to_file(double global_avg, const std::string& filepath) {
    ...
    results_file << "Global Average so2 Concentration: " << global_avg << std::endl;
}

// Function to check for high so2 concentration and alert
void alert_high_so2_concentration(double global_avg, double threshold = 3.0) {
    if (global_avg > threshold) {
        std::cout << "Alert: High so2 concentration detected: " << global_avg << std::endl;
    }
}
// Function to read so2 data from a CSV file
std::vector<double> read_so2_data(const std::string& filepath, int targetColumn) {
    std::vector<double> so2Values;
...
int main(int argc, char** argv) {
    ...
    if (shm_rank == 0) {
        ...
        if (world_rank == 0) {
            int targetColumn = 22;
            allData = read_so2_data("preprocessed_for_cpp.csv", targetColumn);
    ...
    if (world_rank == 0) {
        ...
        std::cout << "Global Average so2 Concentration: " << global_avg << std::endl;
        
        write_global_average_to_file(global_avg, "SO2_analysis_results.txt");

        alert_high_so2_concentration(global_avg);
    }
