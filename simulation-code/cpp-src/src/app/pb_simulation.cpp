...
void write_global_average_to_file(double global_avg, const std::string& filepath) {
    ...
    results_file << "Global Average pb Concentration: " << global_avg << std::endl;
}

// Function to check for high pb concentration and alert
void alert_high_pb_concentration(double global_avg, double threshold = 0.5) {
    if (global_avg > threshold) {
        std::cout << "Alert: High pb concentration detected: " << global_avg << std::endl;
    }
}
// Function to read pb data from a CSV file
std::vector<double> read_pb_data(const std::string& filepath, int targetColumn) {
    std::vector<double> pbValues;
...
int main(int argc, char** argv) {
    ...
    if (shm_rank == 0) {
        ...
        if (world_rank == 0) {
            int targetColumn = 21;
            allData = read_pb_data("preprocessed_for_cpp.csv", targetColumn);
    ...
    if (world_rank == 0) {
        ...
        std::cout << "Global Average PB Concentration: " << global_avg << std::endl;
        
        write_global_average_to_file(global_avg, "pb_analysis_results.txt");

        alert_high_pb_concentration(global_avg);
    }
