...
void write_global_average_to_file(double global_avg, const std::string& filepath) {
    ...
    results_file << "Global Average co Concentration: " << global_avg << std::endl;
}

// Function to check for high co concentration and alert
void alert_high_co_concentration(double global_avg, double threshold = 0.9) {
    if (global_avg > threshold) {
        std::cout << "Alert: High co concentration detected: " << global_avg << std::endl;
    }
}
// Function to read co data from a CSV file
std::vector<double> read_co_data(const std::string& filepath, int targetColumn) {
    std::vector<double> coValues;
...
int main(int argc, char** argv) {
    ...
    if (shm_rank == 0) {
        ...
        if (world_rank == 0) {
            int targetColumn = 20;
            allData = read_co_data("preprocessed_for_cpp.csv", targetColumn);
    ...
    if (world_rank == 0) {
        ...
        std::cout << "Global Average co Concentration: " << global_avg << std::endl;
        
        write_global_average_to_file(global_avg, "co_analysis_results.txt");

        alert_high_co_concentration(global_avg);
    }
